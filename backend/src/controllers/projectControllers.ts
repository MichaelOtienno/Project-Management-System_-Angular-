...
import { projectAssignmentValidationSchema, projectUpdateValidationSchema } from '../validators/projectValidators'
...
export const getProjectStatus = async (req: Request, res: Response) => {
    try {
        const { AssignedUserEmail} = req.query;
        const pool = await mssql.connect(sqlConfig);
...
export const updateProject = async (req: Request, res: Response) => {
    try {
        let { projectID, projectName, projectDescription, endDate, AssignedUserEmail, AssignedUserName } = req.body;

        const { error } = projectUpdateValidationSchema.validate(req.body);
        ...
        if (assignmentResult === -1) {
            return res.status(400).json({ error: 'User is already assigned to an active project' });
        } else {
            const updatedProjectID = projectDetails.recordset[0].UpdatedProjectID;
            return res.status(200).json({
                message: 'Project updated successfully',
                updatedProjectID,
            });
